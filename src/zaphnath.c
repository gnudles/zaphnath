...
A+=B; C^=A; C=ROTL64(C,R)^T;C-=ROTL64(B,15)

#define REV_ZPN_DIFFUSE_ROUND(A, B, C, R, T)\
C+=ROTL64(B,15);C=ROTL64((C^T),64-R);C^=A; A-=B

#define ZPN_DIFFUSE256(A,B,C,D,I,J,K,L) \
ZPN_DIFFUSE_ROUND(D,A,B,25,I);\
ZPN_DIFFUSE_ROUND(C,B,A,11,J);\
ZPN_DIFFUSE_ROUND(C,D,B,51,K);\
ZPN_DIFFUSE_ROUND(A,B,D,48,L);

#define REV_ZPN_DIFFUSE256(A,B,C,D,I,J,K,L) \
REV_ZPN_DIFFUSE_ROUND(A,B,D,48,L);\
REV_ZPN_DIFFUSE_ROUND(C,D,B,51,K);\
REV_ZPN_DIFFUSE_ROUND(C,B,A,11,J);\
REV_ZPN_DIFFUSE_ROUND(D,A,B,25,I);
...
{
	uint64_t b1=0x4c6ba2e9fff0000f;
	uint64_t b2=0x53a63a2bf0f0ff00;
...
void zpn_encrypt(uint64_t nounce, uint64_t counter, struct zpn_key *key, uint8_t *data, uint8_t *out)
{
	...
	uint32_t i, b;
#define o64 ((uint64_t*)out)
	o64[0]=((uint64_t*)data)[0]^counter_df[0];
	o64[1]=((uint64_t*)data)[1]^counter_df[1];
	o64[2]=((uint64_t*)data)[2]^counter_df[2];
	o64[3]=((uint64_t*)data)[3]^counter_df[3];
	uint64_t temp[4];
...
#define ROTF 13
		...
		o64[0]=(temp[0]<<ROTF)|(temp[3]>>(64-ROTF));
		o64[1]=(temp[1]<<ROTF)|(temp[0]>>(64-ROTF));
		o64[2]=(temp[2]<<ROTF)|(temp[1]>>(64-ROTF));
		o64[3]=(temp[3]<<ROTF)|(temp[2]>>(64-ROTF));
		zpn_mat(o64,temp);
		zpn_mixbit(temp,o64);
	}
...
#undef o64
	...
	uint64_t temp[4];
	for (i = key->cycles  ; i>=1 ; --i)
	{
		zpn_mixbit(d64,temp);
		zpn_imat(temp,d64);
#define ROTF 13
		temp[0]=(d64[0]>>ROTF)|(d64[1]<<(64-ROTF));
		temp[1]=(d64[1]>>ROTF)|(d64[2]<<(64-ROTF));
		temp[2]=(d64[2]>>ROTF)|(d64[3]<<(64-ROTF));
		temp[3]=(d64[3]>>ROTF)|(d64[0]<<(64-ROTF));

		zpn_imat(temp,d64);

		REV_ZPN_DIFFUSE256(d64[0], d64[1], d64[2], d64[3],key->cxor[i-1][0],
		key->cxor[i-1][1],key->cxor[i-1][2],key->cxor[i-1][3]);
	}
	uint64_t counter_df[4];
	counter_df[0]=counter;
	counter_df[1]=key->counter_mask[0];
